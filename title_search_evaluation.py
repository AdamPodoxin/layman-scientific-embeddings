...
def get_scores(model: str | Path | SentenceTransformer, top_k_titles=TOP_K_TITLES):
    ...
    query_df, corpus_df = build_layman_document_search_dfs("layman-title")

    doc_id_to_corpus_id = {
        doc_id: corpus_id
        for corpus_id, doc_id in enumerate(corpus_df["doc_id"])
    }
    top_k = min(top_k_titles, corpus_df.shape[0])

    title_embeddings = model.encode_document(corpus_df["document"].to_list())
